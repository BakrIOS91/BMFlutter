...
class PreferencesGenerator extends GeneratorForAnnotation<GeneratePreferences> {
  ...
  void _generateAbstractGetters(
    ...
    for (final field in classElement.fields) {
      ...
      final inApp = _getAnnotation(field, 'InApp');
      if (userDefault == null && secure == null && inApp == null) continue;
    ...
    for (final field in classElement.fields) {
      if (!field.isPrivate ||
          (_getAnnotation(field, 'UserDefault') == null &&
              ...
              _getAnnotation(field, 'InApp') == null)) {
        continue;
      }
  ...
  void _generateGettersAndSetters(
    ...
    for (final field in classElement.fields) {
      ...
      // SETTER: Update the Proxy -> Trigger Reactive UI -> (Optional) Persist to Disk.
      ...
      // Note: inApp doesn't need persistence logic here.
      buffer.writeln('  }');
    }
  }
  ...
  ConstantReader? _getAnnotation(Element element, String name) {
    final checker = TypeChecker.fromUrl(
      'package:bm_flutter/src/helpers/preferences/annotations.dart#$name',
    );
